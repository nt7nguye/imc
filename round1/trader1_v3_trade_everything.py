...
class Trader:
    ...
    def get_fair_value(
        ...
        if (
            ...
            # Filter orders based on adverse volume
            filtered_ask = [
                price
                for price in order_depth.sell_orders.keys()
    ...
    def get_kelp_orders(self, state: TradingState, persistentData: dict) -> List[Order]:
        kelp_position = state.position.get(Product.KELP, 0)
        kelp_fair_value = self.get_fair_value(Product.KELP, state, persistentData)
    ...
    def get_squid_ink_orders(
        ...
        squid_ink_fair_value = self.get_fair_value(
            Product.SQUID_INK, state, persistentData
        )
